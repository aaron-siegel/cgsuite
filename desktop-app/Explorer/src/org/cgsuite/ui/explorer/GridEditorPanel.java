...
public class GridEditorPanel extends EditorPanel
{
     ...
     */
    public GridEditorPanel(CgscriptClass type, Object ruleset, Grid initialGrid, Seq<Icon> icons)
    {
        super();
        ...
        /*
        if (initialGrid instanceof Strip)
            this.permissions.remove(Permission.RESIZE_VERTICAL);
        */
        this.icons = icons;
        cellSize = GridOutput.iconDimensions(this.icons, true);
        setGrid(initialGrid);
    ...
    public Dimension getPreferredSize()
    {
        return new Dimension(
            gridInsets.left + gridInsets.right + grid.colCount() * (3+cellSize.width) + 1,
            gridInsets.top + gridInsets.bottom + grid.rowCount() * (3+cellSize.height) + 1
            );
    }
     ...
     */
    public void setIcons(Seq<Icon> icons)
    {
        this.icons = icons;
        cellSize = GridOutput.iconDimensions(icons, true);
        revalidate();
        repaint();
    }
    ...
    {
        if (row < 0 || row >= grid.rowCount() || grid.rowCount() == 1)
        {
            throw new IllegalArgumentException("row");
        }
        Grid newGrid = Grid$.MODULE$.apply(grid.rowCount()-1, grid.colCount());
        newGrid = newGrid.paste(grid, 1, 1, row, grid.colCount(), 1, 1);
        newGrid = newGrid.paste(grid, row+2, 1, grid.rowCount(), grid.colCount(), row+1, 1);
        setGrid(newGrid);
    ...
    {
        if (col < 0 || col >= grid.colCount() || grid.colCount() == 1)
        {
            throw new IllegalArgumentException("col");
        }
        Grid newGrid = Grid$.MODULE$.apply(grid.rowCount(), grid.colCount()-1);
        newGrid = newGrid.paste(grid, 1, 1, grid.rowCount(), col, 1, 1);
        newGrid = newGrid.paste(grid, 1, col+2, grid.rowCount(), grid.colCount(), 1, col+1);
        setGrid(newGrid);
        firePropertyChange(EDIT_STATE_PROPERTY, Boolean.TRUE, Boolean.FALSE);
        revalidate();
    }
    ...
    public void addRow(int index)
    {
        ...
        Grid newGrid = Grid$.MODULE$.apply(grid.rowCount()+1, grid.colCount());
        newGrid = newGrid.paste(grid, 1, 1, index, grid.colCount(), 1, 1);
        newGrid = newGrid.paste(grid, index+1, 1, grid.rowCount(), grid.colCount(), index+2, 1);
        setGrid(newGrid);
        firePropertyChange(EDIT_STATE_PROPERTY, Boolean.TRUE, Boolean.FALSE);
        revalidate();
    }
    ...
    {
        if (index < 0 || index > grid.colCount())
        {
            throw new IllegalArgumentException("index");
        }
        Grid newGrid = Grid$.MODULE$.apply(grid.rowCount(), grid.colCount()+1);
        newGrid = newGrid.paste(grid, 1, 1, grid.rowCount(), index, 1, 1);
        newGrid = newGrid.paste(grid, 1, index+1, grid.rowCount(), grid.colCount(), 1, index+2);
        setGrid(newGrid);
        firePropertyChange(EDIT_STATE_PROPERTY, Boolean.TRUE, Boolean.FALSE);
        revalidate();
    }
    ...
    {
        return grid.rowCount() > 1 && permissions.contains(Permission.RESIZE_VERTICAL);
    }
    ...
    {
        return grid.colCount() > 1 && permissions.contains(Permission.RESIZE_HORIZONTAL);
    }
    
    protected Icon getIcon(int row, int col)
    {
        return icons.apply(grid.get(row+1, col+1));
    }
    ...
    {
        byte value = grid.get(row+1, col+1);
        do
        {
            value++;
        ...
        } while (value != grid.get(row+1, col+1));
    }
    ...
    public void setCell(int row, int col, byte value)
    {
        ...
        {
            firePropertyChange(EDIT_STATE_PROPERTY, grid.get(row+1, col+1), value);
    ...
    protected void paintComponent(Graphics _g)
    {
        ...
        for (int row = 0; row <= grid.rowCount(); row++)
        {
            ...
            if (allowAddRow(row))
            {
                paintAddButton(g, grid.colCount() * (3+cellSize.width) + 7, row * (3+cellSize.height) - 4);
            }
        }
        for (int col = 0; col <= grid.colCount(); col++)
        {
            ...
            {
                paintAddButton(g, col * (3+cellSize.width) - 4, grid.rowCount() * (3+cellSize.height) + 7);
            }
        }
        for (int row = 0; row < grid.rowCount(); row++)
        {
            ...
            //paintRemoveButton(g, grid.colCount() * (3+cellSize.width) + 7, row * (3+cellSize.height) + (3+cellSize.height)/2 - 4);
        }
        for (int col = 0; col < grid.colCount(); col++)
        {
            ...
            //paintRemoveButton(g, col * (3+cellSize.width) + (3+cellSize.width)/2 - 4, grid.rowCount() * (3+cellSize.height) + 7);
        }
        ...
        for (int row = 0; row <= grid.rowCount(); row++)
        {
            g.drawLine(
                ...
                grid.colCount() * (3+cellSize.width),
                row * (3+cellSize.height)
                );
        }
        for (int col = 0; col <= grid.colCount(); col++)
        {
            g.drawLine(
                ...
                0,
                col * (3+cellSize.width),
                grid.rowCount() * (3+cellSize.height)
                );
        }
        ...
        for (int row = 0; row < grid.rowCount(); row++)
        {
            for (int col = 0; col < grid.colCount(); col++)
            {
                if (grid.get(row+1, col+1) < icons.length() &&
                    g.hitClip(
    ...
    private int hitAddRow(int x, int y)
    {
        ...
        for (int row = 0; row <= grid.rowCount(); row++)
        {
            if (hitSmallRect(grid.colCount() * (3+cellSize.width) + 7, row * (3+cellSize.height) - 4, x, y))
            {
                return row;
            }
        }
    ...
    private int hitAddColumn(int x, int y)
    {
        ...
        for (int col = 0; col <= grid.colCount(); col++)
        {
            if (hitSmallRect(col * (3+cellSize.width) - 4, grid.rowCount() * (3+cellSize.height) + 7, x, y))
            {
                return col;
            }
        }
            ...
            public void mouseReleased(MouseEvent evt)
            {
                ...
                else if (evt.getButton() == MouseEvent.BUTTON1)
                {
                    if (isContainedInGrid(draggingRow, draggingCol))
                    {
                        ...
                        int row = getRow(evt.getY()), col = getColumn(evt.getX());
                        byte value = grid.get(draggingRow+1, draggingCol+1);
                        if (isContainedInGrid(row, col) && allowValue(row, col, value) &&
                            (row != draggingRow || col != draggingCol))
                        {
                            firePropertyChange(EDIT_STATE_PROPERTY, grid.get(draggingRow+1, draggingCol+1), 0);
                            grid = grid.updated(draggingRow+1, draggingCol+1, (byte) 0);
                            firePropertyChange(EDIT_STATE_PROPERTY, grid.get(row+1, col+1), value);
                            grid = grid.updated(row+1, col+1, value);
                            repaint(row, col);
                        }
        ...
        addMouseMotionListener(new MouseMotionAdapter()
        {
            ...
            public void mouseDragged(MouseEvent evt)
            {
                if (isContainedInGrid(draggingRow, draggingCol))
                {
                    ...
                    int row = getRow(evt.getY()), col = getColumn(evt.getX()), value = grid.get(draggingRow+1, draggingCol+1);
                    if (isContainedInGrid(row, col) && allowValue(row, col, value))
                    {
    ...
    private void showPopupMenu(MouseEvent evt)
    {
        ...
        for (byte value = 0; value < icons.length(); value++)
        {
            if (allowValue(row, col, value))
            {
                JMenuItem item;
                if (icons.apply(value) == null)
                {
                    item = new JMenuItem("Empty");
                }
                else
                {
                    item = new JMenuItem(icons.apply(value));
                }
                final byte fVal = value;
    ...
    {
        return type.enclosingClass().get().evalMethod().get().call(
            ruleset,
