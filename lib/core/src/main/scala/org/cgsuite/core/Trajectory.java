...
public class Trajectory implements OutputTarget
{
    private final static RationalNumber[]
        EMPTY_ARRAY = { },
        SINGLE_ZERO_ARRAY = { Values.zero() };

    /** The trajectory with constant value {@link RationalNumber#ZERO}. */
    public final static Trajectory ZERO = new Trajectory(Values.zero());
    /** The trajectory with constant value {@link RationalNumber#POSITIVE_INFINITY}. */
    public final static Trajectory POSITIVE_INFINITY = new Trajectory(Values.positiveInfinity());
    /** The trajectory with constant value {@link RationalNumber#NEGATIVE_INFINITY}. */
    public final static Trajectory NEGATIVE_INFINITY = new Trajectory(Values.negativeInfinity());
    /** The trajectory consisting of a single segment with slope 1 and intercept 0. */
    public final static Trajectory T = ZERO.tilt(Values.one());
    ...
    public Trajectory(RationalNumber mast, RationalNumber[] criticalPoints, RationalNumber[] slopes)
    {
        ...
        if (criticalPoints.length > 0 && criticalPoints[criticalPoints.length-1].compareTo(Values.negativeOne()) <= 0)
        {
        ...
        else
        {
            ...
            for (i = 0; i < criticalPoints.length; i++)
            {
                if (i > 0)
                {
                    value = value.$minus(criticalPoints[i-1].$minus(criticalPoints[i]).$times(slopes[i]));
                }
                xIntercepts[i] = value.$minus(criticalPoints[i].$times(slopes[i]));
            }
            xIntercepts[i] = value.$minus(criticalPoints[i-1].$times(slopes[i]));
        }
    }
    ...
    public RationalNumber valueAt(RationalNumber r)
    {
        ...
        {
            return r.$times(slopes[i]).$plus(xIntercepts[i]);
        }
    }
    ...
    {
        if (valueAt(Values.negativeOne()).compareTo(Values.zero()) <= 0 ||
            valueAt(Values.positiveInfinity()).compareTo(Values.zero()) <= 0)
        {
            return false;
        }
        for (RationalNumber cp : criticalPoints)
        {
            if (valueAt(cp).compareTo(Values.zero()) <= 0)
            {
                return false;
            }
        }
    ...
    public boolean leq(Trajectory t)
    {
        ...
        if (   slopes[0].compareTo(t.slopes[0]) > 0
            ...
            || valueAt(Values.negativeOne()).compareTo(t.valueAt(Values.negativeOne())) > 0)
        {
            return false;
        }
    ...
    private Trajectory minmax(Trajectory t, boolean max)
    {
        ...
        while (true)
        {
                ...
                dominantAtCurrentCP > 0 && dominantAtPrevCP < 0)
            {
                ...
                RationalNumber crossoverPoint = t.xIntercepts[nextCPt].$minus(xIntercepts[nextCPthis]).$div
                    (slopes[nextCPthis].$minus(t.slopes[nextCPt]));
            ...
            if (currentCP.equals(Values.negativeOne()))
            {
                break;
            }
        ...
        int dominantAtTail = maxMultiplier * valueAt(Values.negativeOne()).compareTo(t.valueAt(Values.negativeOne()));
        if (dominantAtTail == 0)
        {
    ...
    public Trajectory tilt(RationalNumber r)
    {
        ...
        for (int i = 0; i < slopes.length; i++)
        {
            result.slopes[i] = slopes[i].$plus(r);
        }
        return result;
    }
    ...
    public Trajectory add(Trajectory t)
    {
        if (isInfinite() || t.isInfinite())
        {
            return new Trajectory(getMastXIntercept().$plus(t.getMastXIntercept()));
        }
        ...
        newSlopes.add(slopes[0].$plus(t.slopes[0]));
        newXIntercepts.add(xIntercepts[0].$plus(t.xIntercepts[0]));
        ...
        while (nextCPthis < criticalPoints.length || nextCPt < t.criticalPoints.length)
        {
            ...
            extendTrajectory(
                ...
                newXIntercepts,
                cp,
                slopeBelow(cp).$plus(t.slopeBelow(cp)),
                interceptBelow(cp).$plus(t.interceptBelow(cp))
                );
    ...
    private static RationalNumber[] negate(RationalNumber[] array)
    {
        ...
        {
            inverse[i] = array[i].unary_$minus();
        }
        return inverse;
    }
    ...
    public Trajectory translate(RationalNumber r)
    {
        ...
        {
            translation.xIntercepts[i] = xIntercepts[i].$plus(r);
        }
        return translation;
    }
    ...
    public RationalNumber firstIntersectionBelow(Trajectory t, RationalNumber theta)
    {
        ...
        if (theta.equals(Values.positiveInfinity()))
        {
            ...
            {
                // They coincide.
                return Values.positiveInfinity();
            }
        }
        else
        {
        ...
        while (nextCPthis <= criticalPoints.length && nextCPt <= t.criticalPoints.length)
        {
            RationalNumber valThis = valueAt
                (nextCPthis == criticalPoints.length ? Values.negativeOne() : criticalPoints[nextCPthis]);
            RationalNumber valOther = t.valueAt
                (nextCPt == t.criticalPoints.length ? Values.negativeOne() : t.criticalPoints[nextCPt]);
            ...
            if (newDominantTrajectory != dominantTrajectory)
            {
                ...
                return t.xIntercepts[nextCPt].$minus(xIntercepts[nextCPthis]).$div
                    (slopes[nextCPthis].$minus(t.slopes[nextCPt]));
            }
        ...
        return Values.negativeInfinity();
    }
    ...
    {
        if (t.compareTo(Values.negativeOne()) < 0)
        {
        ...
        newTrajectory.xIntercepts[firstCPAtOrBelowT+1] = valueAt(t).$minus(newSlope.$times(t));
    ...
    private boolean validate()
    {
        for (int i = 0; i < criticalPoints.length; i++)
        {
            if (!criticalPoints[i].$times(slopes[i]).$plus(xIntercepts[i]).equals
                (criticalPoints[i].$times(slopes[i+1]).$plus(xIntercepts[i+1])))
            {
                return false;
            }
        }
    ...
    public int compareToAt(Trajectory other, RationalNumber t)
    {
        ...
        if (t.equals(Values.positiveInfinity()))
        {
    ...
    public Thermograph thermographicIntersection(Trajectory rightScaffold)
    {
        ...
        RationalNumber lsAtBase = valueAt(Values.negativeOne()),
                 rsAtBase = rightScaffold.valueAt(Values.negativeOne());
            ...
            valueCmp == 0 && slopes[slopes.length-1].compareTo(rightScaffold.slopes[rightScaffold.slopes.length-1]) < 0)
        {
            ...
            if (lsAtBase.compareTo(Values.zero()) > 0)
            {
                previousCaveValue = lsAtBase;
            }
            else if (rsAtBase.compareTo(Values.zero()) < 0)
            {
                previousCaveValue = rsAtBase;
            }
            else
            {
                previousCaveValue = Values.zero();
            }
        }
        else
        {
        ...
        while (nextCPleft >= -1 || nextCPright >= -1)
        {
            ...
            if (nextCPleft == -1 && nextCPright == -1)
            {
                ...
                currentCPowner = 0;
                currentCP = Values.positiveInfinity();
            }
            else
            {
            ...
            else if (previousCaveValue != null)
            {
                ...
                RationalNumber leftScaffoldCrossingPoint =
                    ...
                    ? previousCaveValue.$minus(xIntercepts[nextCPleft+1]).$div(slopes[nextCPleft+1])
                    : null;
                RationalNumber rightScaffoldCrossingPoint =
                    rightScaffold.valueAt(currentCP).compareTo(previousCaveValue) < 0
                    ? previousCaveValue.$minus(rightScaffold.xIntercepts[nextCPright+1])
                        .$div(rightScaffold.slopes[nextCPright+1])
                    : null;
                         ...
                         leftScaffoldCrossingPoint.compareTo(rightScaffoldCrossingPoint) <= 0))
                {
                    ...
                    extendTrajectory(
                        leftWallCPs, leftWallSlopes, leftWallXIntercepts,
                        leftScaffoldCrossingPoint, Values.zero(), previousCaveValue
                        );
                    extendTrajectory(
                        rightWallCPs, rightWallSlopes, rightWallXIntercepts,
                        leftScaffoldCrossingPoint, Values.zero(), previousCaveValue
                ...
                else if (rightScaffoldCrossingPoint != null)
                {
                    ...
                    extendTrajectory(
                        leftWallCPs, leftWallSlopes, leftWallXIntercepts,
                        rightScaffoldCrossingPoint, Values.zero(), previousCaveValue
                        );
                    extendTrajectory(
                        rightWallCPs, rightWallSlopes, rightWallXIntercepts,
                        rightScaffoldCrossingPoint, Values.zero(), previousCaveValue
                ...
                else
                {
                    ...
                    extendTrajectory(
                        leftWallCPs, leftWallSlopes, leftWallXIntercepts,
                        currentCP, Values.zero(), previousCaveValue
                        );
                    extendTrajectory(
                        rightWallCPs, rightWallSlopes, rightWallXIntercepts,
                        currentCP, Values.zero(), previousCaveValue
    ...
    {
        if (newCP.equals(Values.negativeOne()) ||
