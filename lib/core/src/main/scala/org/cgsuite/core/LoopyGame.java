...
import org.cgsuite.exception.NotStopperException;
...
public class LoopyGame
{
    ...
    public LoopyGame(CanonicalShortGame g)
    {
        this();
    ...
    final static LoopyGame ON, OFF, OVER, UNDER;
...
//                ("Growing static markers array for loopy game calculations (new size: " + (len >> 10) + " KB).");
    ...
    static Bigraph initialize(Node startNode)
    {
        ...
        Map<Object,java.lang.Integer> nodeMap = new HashMap<Object,java.lang.Integer>();
        ...
        for (Object o : nodes)
        {
            ...
            {
                int k;
                Iterable<?> leftEdges, rightEdges;
                int leftEdgesSize, rightEdgesSize;      // Java/Scala interop lameness
                if (o instanceof Node)
                {
                    Node node = (Node) o;
                    node.graphInfo = graphInfo;
                    node.startVertex = vertex;
                    List<Object> leftEdgeObjs = new ArrayList<Object>(), rightEdgeObjs = new ArrayList<Object>();
                    ...
                    leftEdgesSize = leftEdgeObjs.size();
                ...
                else if (o instanceof Game)
                {
                    ...
                    leftEdges = JavaConverters.asJavaIterableConverter(loScala).asJava();
    ...
    private static int serializeNodes(List<Object> nodes, Map<Object,java.lang.Integer> nodeMap, Object o, int nextVertexOrdinal)
    {
        ...
        else if (o instanceof CanonicalShortGame)
        {
            ...
            nextVertexOrdinal++;
            for (Game opt : JavaConverters.asJavaIterableConverter(g.leftOptions()).asJava())
            {
    ...
    public Set<LoopyGame> getLeftOptions()
    {
        HashSet<LoopyGame> lo = new HashSet<LoopyGame>();
        for (int i = 0; i < graph.getNumLeftEdges(startVertex); i++)
        {
            LoopyGame gl = new LoopyGame();
            gl.graph = graph;
            gl.startVertex = graph.getLeftEdgeTarget(startVertex, i);
            lo.add(gl);
        }
        return lo;
    }

    public Set<LoopyGame> getRightOptions()
    {
        HashSet<LoopyGame> ro = new HashSet<LoopyGame>();
        for (int i = 0; i < graph.getNumRightEdges(startVertex); i++)
        {
            LoopyGame gr = new LoopyGame();
            gr.graph = graph;
            gr.startVertex = graph.getRightEdgeTarget(startVertex, i);
            ro.add(gr);
        }
        return ro;
    }

    public LoopyGame negative()
    {
        LoopyGame negative = new LoopyGame();
        negative.graph = graph.getInverse();
        negative.startVertex = startVertex;
        return negative;
    }
    /*
    public LoopyGame solve()
    {
        ...
        {
            CanonicalStopperGameImpl solution = new CanonicalStopperGameImpl();
    ...
    public LoopyGame solve(boolean onside)
    {
        ...
        LoopyGame solution = (side.isAlternatingCycleFree(0) ? new CanonicalStopperGameImpl() : new LoopyGame());
        solution.graph = side;
        solution.startVertex = 0;
        return solution;
    }
...
//                    ("Needed " + passes + " passes for the general state-marking algorithm.");
    ...
    LoopyGame canonicalizeStopperInternal()
    {
