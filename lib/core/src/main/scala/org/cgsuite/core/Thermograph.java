...
public class Thermograph implements OutputTarget
{
    ...
    {
        return leftWall.getNumCriticalPoints() == 0 ? Values.negativeOne() : leftWall.getCriticalPoint(0);
    }
    ...
    {
        return rightWall.getNumCriticalPoints() == 0 ? Values.negativeOne() : rightWall.getCriticalPoint(0);
    }
    ...
    public RationalNumber getMast()
    {
        ...
        {
            return rightWall.getSlope(0).equals(Values.zero()) ? rightWall.valueAt(getTemperature()) : Values.positiveInfinity();
        }
        else if (rightWall.equals(Trajectory.NEGATIVE_INFINITY))
        {
            return leftWall.getSlope(0).equals(Values.zero()) ? leftWall.valueAt(getTemperature()) : Values.negativeInfinity();
        }
        else
        {
    ...
    {
        return leftWall.valueAt(Values.positiveInfinity()).compareTo(rightWall.valueAt(Values.positiveInfinity())) > 0;
    }
