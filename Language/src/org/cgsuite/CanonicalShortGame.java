...
public final class CanonicalShortGame extends Game implements Comparable<CanonicalShortGame>
{
    ...
    /**
     * Constructs a string representation of this <code>CanonicalGame</code>.
     * Numbers, nimbers, multiples of up, tinies and minies will be detected
     * and displayed in a condensed form.  Otherwise, this method is very
     * primitive.  For much more flexibility, use a
     * {@link org.cgsuite.ui.CoreIOHandler}.
     *
     * @return  A string representation of this <code>CanonicalGame</code>.
     */
    @Override
    ...
    private static String toString(int id)
    {
        ...
        String s = toString(id, optionHeight, false);
        if (optionHeight[0] == 0)
        {
            return s;
        }
    ...
    private static String toString(int id, int[] height, boolean bracketAll)
    {
        String s;
        ...
        {
            s = "";
            RationalNumber number = getNumberPart(id);
            int upMultiple = getUpMultiplePart(id), nimber = getNimberPart(id);
            if (upMultiple == 0 && nimber == 0 ||
                !number.equals(RationalNumber.ZERO))
            {
                s += number.toString();
            }
            switch (upMultiple)
            {
                ...
                default:
                    if (upMultiple < 0)
                    {
                        s += "v" + String.valueOf(-upMultiple);
                    }
                    else if (upMultiple > 0)
                    {
                        s += "^" + String.valueOf(upMultiple);
                    }
                    break;
            }
            switch (nimber)
            {
                case 0: break;
                case 1: s += "*"; break;
                default: s += "*" + String.valueOf(nimber); break;
            }
            height[0] = 0;
        }
        else if (isNumberTiny(id))
        {
            // A tiny.
            s = "";
            if (getLeftOption(id, 0) != ZERO.id)
            {
                s += toString(getLeftOption(id, 0)) + "+";
            }
            s += "Tiny(" + toString(getInverse(subtract(getRightOption(getRightOption(id, 0), 0), getLeftOption(id, 0)))) + ")";
            height[0] = 0;
        }
        else if (isNumberTiny(getInverse(id)))
        {
            // A miny.
            s = "";
            if (getLeftOption(id, 0) != ZERO.id)
            {
                s += toString(getRightOption(id, 0)) + "+";
            }
            s += "Miny(" + toString(subtract(getLeftOption(getLeftOption(id, 0), 0), getRightOption(id, 0))) + ")";
            height[0] = 0;
        }
        else
        {
            ...
            {
                leftOptionString = toString(getLeftOption(id, 0), optionHeight, bracketAll);
            ...
            else
            {
                ...
                {
                    String t = toString(getLeftOption(id, i), optionHeight, bracketAll);
                    if (optionHeight[0] == 0)
                    {
                        leftOptionString += t;
                    }
            ...
            {
                rightOptionString = toString(getRightOption(id, 0), optionHeight, bracketAll);
            ...
            else
            {
                ...
                {
                    String t = toString(getRightOption(id, i), optionHeight, bracketAll);
                    if (optionHeight[0] == 0)
                    {
                        rightOptionString += t;
                    }
